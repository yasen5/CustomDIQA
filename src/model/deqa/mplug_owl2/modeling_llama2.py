...
# transformers 4.46.3's `from transformers.models.llama.modeling_llama import *` (above) does not
# re-export `_prepare_4d_causal_attention_mask_for_sdpa` (unlike the older transformers this file
# was originally written against), so pull it explicitly from where it actually lives now.
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        # transformers 4.46.3's LlamaRotaryEmbedding.forward(x, position_ids) replaced the older
        # forward(x, seq_len=...) that returned a cached table sliced to `seq_len`. We never pass
        # past_key_value (no KV caching in this scoring-only, single-forward-pass usage), so
        # kv_seq_len == q_len and position_ids already covers the full current sequence -- this is
        # exactly equivalent, not just a shape-compatible substitute.
...
def model_forward(
    ...
    # Older transformers exposed this as self._use_flash_attention_2 / self._use_sdpa booleans set
    # in LlamaModel.__init__; transformers 4.46.3 dropped those in favor of checking
    # self.config._attn_implementation directly, so that's what we check here too.
