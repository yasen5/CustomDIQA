...
#
# Vendored from modeling_mplug_owl2_huggingface.py in the zhiyuanyou/DeQA-Score-Mix3 Hugging
# Face repo, with the following deltas from upstream:
#   - dropped the unused `from icecream import ic` debug-print import
#   - dropped the module-level `AutoConfig.register(...)` / `AutoModelForCausalLM.register(...)`
#     calls and the `if __name__ == "__main__":` smoke test -- this module is imported directly
#     (see .model.DeQAScoreForQuality), never through `AutoModelForCausalLM.from_pretrained(...,
#     trust_remote_code=True)`, so there is nothing to register into the transformers Auto* maps
#   - `replace_llama_modality_adaptive()` (monkeypatches transformers.models.llama.modeling_llama)
#     is called explicitly by .model rather than as an import-time side effect here, so the one
#     process-wide mutation this package performs is visible at the call site instead of hidden
#     in an import
